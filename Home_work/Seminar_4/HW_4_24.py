...
for i in range(n):
    berries.append((random.randint(0,100)))

print('Наша грядка: ', berries)  

sbor = list()

for i in range(len(berries) - 1):
    sbor.append(berries[i-1] + berries[i] + berries[i+1]) # сбор ягод с текущего и двух соседних кустов
    
print('Собрали ягод: ', sbor)
print('Максимальное число ягод: ', max(sbor))